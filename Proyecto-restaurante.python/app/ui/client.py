import tkinter as tk
from tkinter import messagebox, simpledialog
import mysql.connector
from app.config import DB_CONFIG
from app.ui.styles import COLORS, ICONS, FONTS
from app.utils.window_utils import remove_default_icon

# Conexión a la base de datos
try:
	conectar = mysql.connector.connect(**DB_CONFIG)
	base_datos = conectar.cursor()
	print("Conexión exitosa a la base de datos")
except mysql.connector.Error as err:
	print(f"Error al conectar a la base de datos: {err}")
	exit(1)  # Salir si no hay conexión

class MesasClienteGUI:
	def __init__(self, master):
		self.master = master
		master.title(f"{ICONS['cliente']} Reservar Mesas - Cliente")
		master.geometry("900x700")
		master.configure(bg=COLORS['bg_main'])
		# Eliminar icono por defecto usando icono transparente
		from app.utils.window_utils import get_transparent_icon_path
		import os
		transparent_icon = get_transparent_icon_path()
		if transparent_icon and os.path.exists(transparent_icon):
			try:
				master.iconbitmap(transparent_icon)
			except:
				pass
		master.after(100, lambda: remove_default_icon(master))
		
		# Header con título
		header = tk.Frame(master, bg=COLORS['secondary'], height=60)
		header.pack(fill='x')
		header.pack_propagate(False)
		
		titulo = tk.Label(
			header,
			text=f"{ICONS['restaurante']} Reserva de Mesas - Cliente",
			font=FONTS['title'],
			bg=COLORS['secondary'],
			fg=COLORS['text_light']
		)
		titulo.pack(pady=15)
		
		# Frame para las mesas con scroll
		self.canvas = tk.Canvas(master, bg=COLORS['bg_light'], highlightthickness=0)
		self.scrollbar = tk.Scrollbar(master, orient="vertical", command=self.canvas.yview)
		self.scrollable_frame = tk.Frame(self.canvas, bg=COLORS['bg_light'])

		self.scrollable_frame.bind(
			"<Configure>",
			lambda e: self.canvas.configure(
				scrollregion=self.canvas.bbox("all")
			)
		)

		self.canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw")
		self.canvas.configure(yscrollcommand=self.scrollbar.set)

		self.canvas.pack(side="left", fill="both", expand=True)
		self.scrollbar.pack(side="right", fill="y")

		# Cargar las mesas existentes desde la base de datos
		self.cargar_mesas_desde_bd()

	def cargar_mesas_desde_bd(self):
		"""Carga todas las mesas almacenadas en la base de datos"""
		try:
			base_datos.execute("SELECT id_mesa, numero_mesa, nombre_cliente, ubicacion, estado, cantidad_personas FROM mesas")
			mesas_bd = base_datos.fetchall()
			print(f"Mesas obtenidas: {mesas_bd}")  # Para depuración

			if not mesas_bd:
				no_mesas_label = tk.Label(
					self.scrollable_frame,
					text=f"{ICONS['info']} No hay mesas disponibles.",
					font=FONTS['subtitle'],
					bg=COLORS['bg_light'],
					fg=COLORS['text_secondary']
				)
				no_mesas_label.pack(pady=50)
				return

			for mesa_bd in mesas_bd:
				id_mesa, numero_mesa, nombre_cliente, ubicacion, estado, capacidad = mesa_bd
				self.crear_mesa(id_mesa, numero_mesa, nombre_cliente, ubicacion, estado, capacidad)

		except mysql.connector.Error as error:
			messagebox.showerror("Error", f"No se pudo cargar las mesas. Error: {error}")

	def crear_mesa(self, id_mesa, numero_mesa, nombre_cliente, ubicacion, estado, capacidad):
		"""Crea un botón para cada mesa y ajusta su funcionalidad según el estado"""
		estado_lower = estado.lower() if estado else "libre"
		if estado_lower == "libre":
			color = COLORS['mesa_libre']
			estado_icon = "✅"
			comando = lambda: self.reservar_mesa(id_mesa)
			texto_boton = f"{ICONS['mesa']} Mesa {numero_mesa}\n{ICONS['ubicacion']} {ubicacion}\n{ICONS['personas']} {capacidad} personas\n{estado_icon} Disponible"
		else:
			color = COLORS['mesa_reservada']
			estado_icon = "🔴"
			comando = lambda: self.ver_detalles_mesa(id_mesa, numero_mesa, nombre_cliente, ubicacion, estado, capacidad)
			texto_boton = f"{ICONS['mesa']} Mesa {numero_mesa}\n{ICONS['ubicacion']} {ubicacion}\n{ICONS['personas']} {capacidad} personas\n{estado_icon} Reservada\n{ICONS['usuario']} {nombre_cliente or 'Sin cliente'}"

		mesa_button = tk.Button(
			self.scrollable_frame,
			text=texto_boton,
			width=28,
			height=6,
			bg=color,
			fg=COLORS['text_light'],
			font=FONTS['body'],
			relief='raised',
			bd=2,
			cursor='hand2',
			command=comando
		)

		mesa_button.pack(padx=15, pady=15, side=tk.LEFT, fill='both', expand=True)

	def reservar_mesa(self, id_mesa):
		"""Reserva la mesa seleccionada solicitando el nombre del cliente"""
		nombre_cliente = simpledialog.askstring(
			f"{ICONS['mesa']} Reserva de Mesa",
			f"{ICONS['usuario']} Por favor, introduce tu nombre:"
		)

		if nombre_cliente:
			try:
				# Actualizar el estado y el nombre del cliente en la base de datos
				query = "UPDATE mesas SET estado = 'Reservado', nombre_cliente = %s WHERE id_mesa = %s AND estado = 'Libre'"
				base_datos.execute(query, (nombre_cliente, id_mesa))
				conectar.commit()

				if base_datos.rowcount == 0:
					messagebox.showwarning(
						f"{ICONS['warning']} Reserva Fallida",
						f"La mesa ya ha sido reservada por otro cliente."
					)
				else:
					messagebox.showinfo(
						f"{ICONS['exito']} Reserva Exitosa",
						f"¡La mesa ha sido reservada con éxito para {nombre_cliente}!"
					)
					self.actualizar_interfaz()

			except mysql.connector.Error as error:
				messagebox.showerror(
					f"{ICONS['error']} Error",
					f"No se pudo reservar la mesa. Error: {error}"
				)
		else:
			messagebox.showinfo(
				f"{ICONS['info']} Reserva Cancelada",
				"No se ha ingresado un nombre. La reserva ha sido cancelada."
			)

	def ver_detalles_mesa(self, id_mesa, numero_mesa, nombre_cliente, ubicacion, estado, capacidad):
		"""Muestra los detalles de una mesa reservada"""
		detalles = (f"{ICONS['info']} Detalles de la Mesa {numero_mesa}\n\n"
				   f"{ICONS['ubicacion']} Ubicación: {ubicacion}\n"
				   f"{ICONS['personas']} Capacidad: {capacidad} personas\n"
				   f"🔴 Estado: {estado}\n"
				   f"{ICONS['usuario']} Cliente: {nombre_cliente or 'Sin asignar'}")
		messagebox.showinfo(f"{ICONS['mesa']} Detalles de la Mesa", detalles)

	def actualizar_interfaz(self):
		"""Recarga las mesas para reflejar los cambios en la reserva"""
		for widget in self.scrollable_frame.winfo_children():
			widget.destroy()
		self.cargar_mesas_desde_bd()
